...
/*   Updated: 2018/01/18 16:20:08 by skushnir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		draw_fract(t_mlx *data)
{
	...
	while (++x < WIDTH)
	{
		...
		{
			i = -1;
			a = 1.5 * (x - WIDTH / 2) / (0.5 * data->index * WIDTH ) + data->j_x;
...
{
	x = y;
	data->m_act ? data->re -= 0.05 : 0;
	data->m_act ? data->im -= 0.05 : 0;
...
int julia()
{
	...
	data = (t_mlx){.index = 1, .re = -0.70176, .im = -0.3842, .j_x = 0, .j_y = 0,
			.m_act = 0};
	data.mlx = mlx_init();
