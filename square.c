...
/*   Updated: 2018/01/20 11:55:42 by sergee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_square(t_mlx *data)
{
	...
	while (++i < 1200)
	{
		xy[0].x = (int)(data->width / 2 + d[0] * cos(d[1] + M_PI / 4));
		xy[0].y = (int)(data->high / 2 + d[0] * sin(d[1] + M_PI / 4));
		xy[1].x = (int)(data->width / 2 + d[0] * cos(d[1] + 3 * M_PI / 4));
		xy[1].y = (int)(data->high / 2 + d[0] * sin(d[1] + 3 * M_PI / 4));
		xy[2].x = (int)(data->width / 2 + d[0] * cos(d[1] + 5 * M_PI / 4));
...
int			square(void)
{
	...
	data = (t_mlx){.width = WIDTH, .high = HIGH,
		.index = 1, .re = 20, .im = 3};
	data.mlx = mlx_init();
	data.win = mlx_new_window(data.mlx, data.width, data.high, "Square");
